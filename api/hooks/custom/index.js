...
---------------------------------------------------------------------${suffix}`);
        ...
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            if (!req.path.includes('logout')) {
              await sails.helpers.updateOnlineState(req.me);
            }
  ...
  function localize(req, res) {
    // if it is virtual request skip it as we dont have cookies;
    if (req.isSocket) return;

    let { lang } = req.cookies;
    //set cookie on 10 years
    const expires = new Date();
    expires.setTime(expires.getTime() + 365 * 10 * (24*60*60) * 1000);
    lang = lang || req.getLocale();
    req.setLocale(lang);
  ...
  /*async function webHooks(req) {

    const NEW_VIEW_PAGE_URI_TOKEN = 'new-views';

    if (!req || typeof req !== 'object') return;

    const { path, me, session } = req;

    // clear all new views record in Views collection
    if (typeof path === 'string' && me) {

        if (!path.includes(NEW_VIEW_PAGE_URI_TOKEN) && me.newViewsPageVisitedState) {
          const [, newViews] = await Promise.all([
            User.updateOne({ id: session.userId })
                .set({ newViewsPageVisitedState: false }),
            NewView.destroy({ viewedId: session.userId })
                   .fetch()
          ]);
